...
function getEnhancements(answers: FlowAnswers): string {
  ...
  switch (answers.servico) {
    case 'video':
      ...
      break;
    case 'restauracao':
      if (answers.qualidade === 'baixa') enhancements.push('heavy scratch removal, tear repair, color restoration');
      if (answers.qualidade === 'media') enhancements.push('moderate scratch removal, color correction');
      if (answers.qualidade === 'alta') enhancements.push('light restoration, color enhancement');
    ...
    case 'reconstrucao':
      ...
      if (answers.comoVer === 'idade-especifica' && answers.idadeDesejada) {
        enhancements.push('age to approximately ' + answers.idadeDesejada + ' years');
      }
...
function getConstraints(answers: FlowAnswers): string {
  const constraints: string[] = [
    'no distortion', 'no artificial faces', 'no extra fingers',
    'no blur artifacts', 'preserve original identity',
  ];
